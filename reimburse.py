...
class Trip:
    ...
    def __post_init__(self):
        ...
        occupancy_multiplier: float = self.num_occupants / self.config["max_occupants"][self.vehicle_class]
        occupancy_multiplier = min(1.0, occupancy_multiplier)

        actual_mpg: float = self.config["total_miles"] / self.actual_cost * self.config["gas_per_gal"]
        mpg_multiplier: float = actual_mpg / self.config["expected_mpg"][self.vehicle_class]
        mpg_multiplier = min(1.0, mpg_multiplier)

        expected_gallons_consumed: float = self.config["total_miles"] / self.config["expected_mpg"][self.vehicle_class]
        self.expected_cost: float = self.config["gas_per_gal"] * expected_gallons_consumed
        self.reimbursed: float = occupancy_multiplier * mpg_multiplier * self.expected_cost
